...
solver = pywraplp.Solver("The Stigler diet",pywraplp.Solver.GLOP_LINEAR_PROGRAMMING)
...
# 결정변수 정의
x = [solver.NumVar(0.0,solver.infinity(),food[0]) for food in data]
...
for i, nutrient in enumerate(nutrients):
  ...
  for j, food in enumerate(data):
    constraints[i].SetCoefficient(x[j],food[i+3])
print("Number of constraints =", solver.NumConstraints())


# 목적함수 정의
obj = solver.Objective()
for k , food in enumerate(data):
  obj.SetCoefficient(x[k],food[2])
...
if status == pywraplp.Solver.OPTIMAL:
  print("Solution:")
  print(f"Objective value = {365.* solver.Objective().Value():.2f}$")
  for j in range(0,len(data)):
    print(f"The purchase amount of food {x[j]} ={365.*x[j].solution_value()}$",)
else:
  print("The problem does not have an optimal solution")
